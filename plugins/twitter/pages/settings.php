<?php
	...
	if ($func == '') {
		//Start - generate list for accounts
			...
			$list->setNoRowsMessage($this->i18n('twitter_settings_accounts_norowsmessage'));
			...
			$list->setColumnLabel('title', $this->i18n('twitter_settings_accounts_column_title'));
			
			// functions column spans 2 data-columns
			$funcs = $this->i18n('twitter_settings_accounts_column_functions');
			...
			//Start - output list via fragment
				...
				$fragment->setVar('title',  $this->i18n('twitter_settings_accounts'), false);
		...
		//Start - generate list for hashtags
			$list = rex_list::factory("SELECT `id`, `hashtag` FROM `".rex::getTablePrefix()."socialhub_twitter_hashtag` ORDER BY `hashtag` ASC");
			$list->addTableAttribute('class', 'table-striped');
			$list->setNoRowsMessage($this->i18n('twitter_settings_hashtags_norowsmessage'));
			...
			$list->setColumnLabel('hashtag', $this->i18n('twitter_settings_hashtags_column_hashtag'));
			
			// functions column spans 2 data-columns
			$funcs = $this->i18n('twitter_settings_hashtags_column_functions');
			...
			//Start - output list via fragment
				$fragment = new rex_fragment();
				$fragment->setVar('class', 'info', false);
				$fragment->setVar('title',  $this->i18n('twitter_settings_hashtags'), false);
				$fragment->setVar('content', $content, false);
				echo $fragment->parse('core/page/section.php');
			//End - output list via fragment
		...
		//Start - generate list for timelines
			$list = rex_list::factory("SELECT `id`, `title` FROM `".rex::getTablePrefix()."socialhub_twitter_timeline` ORDER BY `title` ASC");
			$list->addTableAttribute('class', 'table-striped');
			$list->setNoRowsMessage($this->i18n('twitter_settings_timelines_norowsmessage'));
			...
			$list->setColumnLabel('title', $this->i18n('twitter_settings_timelines_column_title'));
			
			// functions column spans 2 data-columns
			$funcs = $this->i18n('twitter_settings_timelines_column_functions');
			...
			//Start - output list via fragment
				$fragment = new rex_fragment();
				$fragment->setVar('class', 'info', false);
				$fragment->setVar('title',  $this->i18n('twitter_settings_timelines'), false);
				$fragment->setVar('content', $content, false);
				echo $fragment->parse('core/page/section.php');
			//End - output list via fragment
		...
		if ($func == 'editAccount') {
			$formLabel = $this->i18n('twitter_settings_accounts_formcaption_edit');
		} elseif ($func == 'addAccount') {
			$formLabel = $this->i18n('twitter_settings_accounts_formcaption_add');
		}
		...
		if ($func == 'editHashtag') {
			$formLabel = $this->i18n('twitter_settings_hashtags_formcaption_edit');
		} elseif ($func == 'addHashtag') {
			$formLabel = $this->i18n('twitter_settings_hashtags_formcaption_add');
		}
		
		$form = rex_form::factory(rex::getTablePrefix().'socialhub_twitter_hashtag', '', 'id='.$id);
		
		//Start - add hashtag-field
			$field = $form->addTextField('hashtag');
			$field->setLabel($this->i18n('twitter_settings_hashtags_label_hashtag'));
		...
		$del = rex_sql::factory();
		$del->setQuery('DELETE FROM ' . rex::getTablePrefix() . 'socialhub_twitter_hashtag WHERE `id` = "'.$id.'"');
		echo 'Hashtag wurde gelöscht'; //todo: translate
	} else if ($func == 'addTimeline' || $func == 'editTimeline') {
		$id = rex_request('id', 'int');
		
		if ($func == 'editTimeline') {
			$formLabel = $this->i18n('twitter_settings_timelines_formcaption_edit');
		} elseif ($func == 'addTimeline') {
			$formLabel = $this->i18n('twitter_settings_timelines_formcaption_add');
		}
		
		$form = rex_form::factory(rex::getTablePrefix().'socialhub_twitter_timeline', '', 'id='.$id);
		
		//Start - add title-field
			$field = $form->addTextField('title');
			$field->setLabel($this->i18n('twitter_settings_timelines_label_title'));
		//End - add title-field
		
		//Start - add user_id-field
			$field = $form->addTextField('user_id');
			$field->setLabel($this->i18n('twitter_settings_timelines_label_user_id'));
		...
		$del = rex_sql::factory();
		$del->setQuery('DELETE FROM ' . rex::getTablePrefix() . 'socialhub_twitter_timeline WHERE `id` = "'.$id.'"');
