...
foreach($sh->entries(true,array('highlight'=>1)) as $key => $value) {
  ...
  $fragment->setVar('title', 'Seite: <a href="https://www.twitter.com/'.$value['query']['user']['username'].'" target="_blank" title="Instagram '.$value['query']['user']['username'].' aufrufen">'.ucfirst($value['query']['user']['username']).'</a>',false);
