<?php
	...
	if ($func == '') {
		$list = rex_list::factory("SELECT `id`, `message` FROM `".rex::getTablePrefix()."socialhub_entry_timeline` WHERE `source` = 'instagram' ORDER BY `id` DESC");
		$list->addTableAttribute('class', 'table-striped');
		$list->setNoRowsMessage($this->i18n('entries_norowsmessage'));
		
		// icon column
		$list->addColumn('&nbsp;', '<i class="rex-icon fa-instagram"></i>', 0, ['<th class="rex-table-icon">###VALUE###</th>', '<td class="rex-table-icon">###VALUE###</td>']);
